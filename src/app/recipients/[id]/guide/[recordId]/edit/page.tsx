...
export default function EditGuideRecordPage() {
  ...
  const updateMutation = trpc.guideRecord.update.useMutation();
  ...
  useEffect(() => {
    if (record) {
      ...
      reset({
        startedAt: toLocalDatetimeString(new Date(record.startedAt)),
        endedAt: record.endedAt
          ? toLocalDatetimeString(new Date(record.endedAt))
          : "",
        ...
        cashHandled: record.cashHandled,
  ...
  const onSubmit = async (data: GuideFormData) => {
    try {
      ...
      router.push(`/recipients/${recipientId}/guide/${recordId}`);
    } catch (error) {
      console.error("ガイド記録エラー:", error);
      alert("ガイド記録の更新に失敗しました");
    }
  };
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-8">
      <Link href={`/recipients/${recipientId}/guide/${recordId}`}>
      ...
      <h1 className="mb-2 text-3xl font-bold">ガイド記録編集</h1>
      {recipient && (
        <p className="text-muted-foreground mb-8">
          {recipient.name} さんのガイド記録を編集します
        </p>
      )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex gap-4">
          <Button
            ...
            className="flex-1"
          >
            {updateMutation.isPending ? "更新中..." : "ガイド記録を更新"}
          </Button>
          <Link
            href={`/recipients/${recipientId}/guide/${recordId}`}
            className="flex-1"
          >
            <Button
              type="button"

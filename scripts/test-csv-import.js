  ...
  try {
    ...
    if (result.recipients.length > 0) {
      ...
      const supportProfileFields = [
        'disabilityName', 'diseaseStatus', 'hasSeizures', 'medication', 'healthNote',
        'favoriteFoods', 'dislikedFoods', 'hasAllergy', 'allergyNote',
        'mobilityMethod', 'mobilityAssist', 'mobilityNote',
        'commVerbal', 'commGesture', 'commNote'
      ];
      ...
      supportProfileFields.forEach(field => {
        ...
        if (value !== null && value !== undefined) {
          if (Array.isArray(value)) {
            status = value.length > 0 ? `✅ ${JSON.stringify(value)}` : '⚠️  EMPTY ARRAY';

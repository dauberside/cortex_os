...
async function runPhase2Migration() {
  ...
  try {
    ...
    console.log(
      "  - documentCreatedDate, organizationName, documentHeaderGroupHomeName"
    );
    ...
    console.log(
      "  - disabilityPensionAmount, publicMedicalCare, medicalInsuranceTypes (JSON)"
    );
    console.log(
      "  - medicalInsuranceDetail, disabilityCause, disabilityDetail"
    );
    console.log("  - lifeHistory, finalEducation, workHistory");
    console.log("  - hospitalVisits, regularCheckups, medicalRemarks");
    console.log(
      "  - currentServices (JSON), guardianshipType, welfareRightsAdvocacy"
    );
    console.log(
      "  - welfareRightsDetail, specialRemarks, contactPolicy (JSON)\n"
    );
    ...
    console.log(
      `Added columns: ${afterCount.rows[0].count - beforeCount.rows[0].count}\n`
    );
